...
public class TraceLogger : ILogger
{
    ...
    {
        Trace.WriteLine(string.Format("INFO [{1}] {0}", string.Format(format, args), _name), "Stylet");
    }
    ...
    {
        Trace.WriteLine(string.Format("WARN [{1}] {0}", string.Format(format, args), _name), "Stylet");
    }
    ...
    public void Error(Exception exception, string? message = null)
    {
        if (message == null)
        {
            Trace.WriteLine(string.Format("ERROR [{1}] {0}", exception, _name), "Stylet");
            return;
        }
        Trace.WriteLine(string.Format("ERROR [{2}] {0} {1}", message, exception, _name), "Stylet");
    }
}
